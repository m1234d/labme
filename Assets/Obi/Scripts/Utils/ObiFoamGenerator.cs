	...
	public class ObiFoamGenerator : MonoBehaviour
	{
		...
		private void Subscribe(){
			if (isActiveAndEnabled && !subscribed && emitter.Solver != null){
				subscribed = true;
				emitter.Solver.OnFrameEnd += Emitter_Solver_OnFrameEnd;
			}
		}
		...
		void Emitter_Solver_OnFrameEnd (object sender, EventArgs e)
		{
			...
			for (int i = 0; i < emitter.particleIndices.Length; ++i){
				...
				float vorticity = emitter.Solver.vorticities[k].sqrMagnitude;

				if (vorticity > vorticityThreshold*vorticityThreshold && emitter.Solver.fluidData[k][0] < densityThreshold){
